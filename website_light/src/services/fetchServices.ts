 ...
 */
  export const searchSymbol = async (query: string) => {
    const url = `${finnhubBaseURL}/search?q=${query}&token=${process.env.REACT_APP_API_KEY}`;
...
export const fetchStockDetails = async (stockSymbol: string) => {
    const url = `${finnhubBaseURL}/stock/profile2?symbol=${stockSymbol}&token=${process.env.REACT_APP_API_KEY}`;
    const response = await fetch(url);
...
export const fetchQuote = async (stockSymbol: string) => {
    const url = `${finnhubBaseURL}/quote?symbol=${stockSymbol}&token=${process.env.REACT_APP_API_KEY}`;
    const response = await fetch(url);
...
export const fetchHistoricalData = async (
    ...
    const url = `${finnhubBaseURL}/stock/candle?symbol=${stockSymbol}&resolution=${resolution}&from=${from}&to=${to}&token=${process.env.REACT_APP_API_KEY}`;
    const response = await fetch(url);
